...
class IndependentReserve : ExchangeAbstract, Exchange
{
    //MARK:- class constants
    
    let keychainKeyForApiKey: String = "au.com.addisonbrown.api.independentreserve.key"
    let keychainKeyForApiSecret: String = "au.com.addisonbrown.api.independentreserve.secret"
    ...
    {
        IndependentReserveRouter.apiKey = ApiKey(key: apiKey, secret: apiSecret)
    }
    ...
    func getTicker(instrument: Instrument, callback: (ticker: Ticker?, error: NSError?) -> () )
    {
        ...
        Alamofire.request(IndependentReserveRouter.Ticker(instrument))
            .responseJSON { (request, response, json, error) in
                ...
                else if let json: AnyObject = json
                {
                    ...
                    if  let ask = json["CurrentLowestOfferPrice"].double,
                        let bid = json["CurrentHighestBidPrice"].double,
                        let lastPrice = json["LastPrice"].double,
                        let timestamp = json["CreatedTimestampUtc"].string
                    {
    ...
    func getOrderBook(instrument: Instrument, callback: (orderBook: OrderBook?, error: NSError?) -> () )
    {
        ...
        Alamofire.request(IndependentReserveRouter.OrderBook(instrument))
            .responseJSON { (request, response, json, error) in
    ...
    private func convertOrdersInOrderBook(orders: JSON) -> [OrderBookOrder]
    {
        ...
        for (index: String, order: JSON) in orders
        {
            ...
            if  let price = order["Price"].double,
                let quantity =  order["Volume"].double
            {
    ...
    {
        Alamofire.request(IndependentReserveRouter.Trades(instrument))
            .responseJSON { (request, response, JSON, error) in
...
//                    callback(trades: [newTrade], error: nil)
    ...
    func getBalances(callback: (balances: [Balance]?, error: NSError?) -> () )
    {
        ...
        Alamofire.request(IndependentReserveRouter.Balances())
            .responseJSON { (request, response, data, error) in
                ...
                else if let data: AnyObject = data
                {
                    ...
                    if let exchangeBalances = json.array
                    {
                        ...
                        {
                            if let rawCurrencyCode: String = balance["CurrencyCode"].string?.uppercaseString
                            {
                                var currencyCode = rawCurrencyCode
                                if rawCurrencyCode == "XBT"
                                {
                                    currencyCode = "BTC"
                                }
                                ...
                                if let currency = CurrencyManager.sharedCurrencyManager.find(currencyCode)
                                {
                                    ...
                                    if let total: Double = balance["TotalBalance"].double,
                                        let available: Double = balance["AvailableBalance"].double
                                    {
                                        newBalance.total = total
                                        newBalance.available = available
