...
public class Order: SuperOrder
{
    ...
    public var vwap: Double
        {
            var amountPrice: Double = 0
            var amount: Double = 0
            
            for trade in self._trades
            {
                amount += trade.amount
                amountPrice += trade.amount * trade.price
            }
            
            return amountPrice / amount
    }
    ...
    public func addTrade(price: Double, amount: Double) -> NSError?
    {
        var trade = Trade(exchange: exchange, instrument: instrument, type: type, side: side, amount: amount, price: price)
    ...
    public var amountFilled: Double
        {
            ...
            return _trades.reduce(0) {$0 + $1.amount}
    }
