...
*/
    ...
    @IBAction func calculateFib(sender: AnyObject)
    {
        ...
        {
            resultField.text = "Please enter an integer"
        }
        else
        {
            enteredNumber = numberField.text.toInt()
            
            if (enteredNumber == nil)
            {
                resultField.text = "You must enter a integer"
            }
            else
            {
                var fibNumber = fibAdder.fibonacciNumberAtIndex(enteredNumber!)
                
                resultField.text = String(fibNumber)
            }
        }
    }
}
